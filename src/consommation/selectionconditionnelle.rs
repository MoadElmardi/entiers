use tfhe::prelude::*;
use tfhe::{generate_keys, set_server_key, ConfigBuilder, FheUint16, FheBool};
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ctxt_cond = FheBool::try_encrypt(false, &client_key)?;
    let ctxt_max = ctxt_cond.if_then_else(&ctxt_a, &ctxt_b);
